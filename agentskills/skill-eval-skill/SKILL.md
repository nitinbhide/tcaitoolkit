...
1.  **Parse the Evaluation File**: Read the specified markdown file. The file contains one or more evaluation cases. Each case is separated by one or more blank lines and follows this exact format:
    ...
    **prompt**: {the prompt to send to the target skill. The prompt may use the list of filenames provided in the input}
...
2.  **Execute Evaluations**: For each evaluation case you parse from the file:
    ...
    b.  Invoke the target skill, passing it the `prompt` from the test case.
