...
- Mandatory reference usage:
  - Use the repo-nav ripgrep guidance when `rg` is available.
  - Use the repo-nav PowerShell guidance for path handling and validations on Windows.
  - Use the repo-nav Bash guidance for path handling and validations on Unix-like systems.
- Template compliance is required:
  - Root index must use the repo-nav root index template.
  - Folder indexes must use the repo-nav folder index template.
  - Indexing plan must use the repo-nav planning template.
